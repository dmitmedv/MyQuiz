...
function getUserSettings(userId: number): Promise<UserSettings | null> {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT id, user_id, selected_languages, created_at, updated_at FROM user_settings WHERE user_id = ?',
      [userId],
      (err, row: any) => {
        if (err) {
          reject(err);
  ...
  try {
    ...
    if (!settings) {
      ...
      const settingsId = await new Promise<number>((resolve, reject) => {
        db.run(
          'INSERT INTO user_settings (user_id, selected_languages) VALUES (?, ?)',
          [userId, JSON.stringify(defaultLanguages)],
          function (err) {
            if (err) reject(err);
...
router.put('/settings', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const userId = req.user.id;
    const { selected_languages }: UpdateUserSettingsRequest = req.body;
    ...
    if (existingSettings) {
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'UPDATE user_settings SET selected_languages = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?',
          [JSON.stringify(selected_languages), userId],
          (err) => {
            if (err) reject(err);
            else resolve();
          }
      ...
      await new Promise<void>((resolve, reject) => {
        db.run(
          'INSERT INTO user_settings (user_id, selected_languages) VALUES (?, ?)',
          [userId, JSON.stringify(selected_languages)],
          (err) => {
            if (err) reject(err);
            else resolve();
          }
